...
{
	public static class FourBitEncoder
	{
		private static int[] IndexTable_4Bit = new int[] { -1, -1, -1, -1, 2, 4, 6, 8, -1, -1, -1, -1, 2, 4, 6, 8 };
		private static int[] StepSizeTable_4Bit = new int[] {
		...
		public static double[] Encode(double[] speechFile)
		{
			...
			var prevsample = 0;
			var previndex = 1;
			var Ns = speechFile.Length;
			var n = 1;
			var result = new List<double>(speechFile.Length);
			var data = Real2Hex.Number_Real2Hex(speechFile, 15, 16);
			speechFile = Hex2Real.Number_Hex2Real(data, 0, 16);
			for (int i = 0; i < Ns; i++)
			{
				var predsample = prevsample;
				var index = previndex;

				var step = StepSizeTable_4Bit[index - 1];
				...
				index = index + IndexTable_4Bit[(int)code];

				if (index < 1)
				{
					index = 1;
				}
				else if (index >= StepSizeTable_4Bit.Length)
				{
					index = StepSizeTable_4Bit.Length - 1;
				}
